...
// Shown only when round_type === 'CODING'. Lets the company author a DSA problem.
function CodingProblemEditor({ assessmentId }) {
  const BLANK = {
    title: '', problemStatement: '', constraintsText: '', inputFormat: '',
    outputFormat: '', sampleInput: '', sampleOutput: '',
    allowedLanguages: ['python', 'java', 'cpp'], timeLimitMs: 2000, memoryLimitMb: 256, maxScore: 100,
  }
  ...
  useEffect(() => {
    let live = true
    getCodingProblem(assessmentId).then(prob => {
      if (!live) return
      if (prob) {
        setExisting(prob)
        setForm({
          title: prob.title ?? '',
          problemStatement: prob.problem_statement ?? '',
          constraintsText: prob.constraints_text ?? '',
          inputFormat: prob.input_format ?? '',
          outputFormat: prob.output_format ?? '',
          sampleInput: prob.sample_input ?? '',
          sampleOutput: prob.sample_output ?? '',
          allowedLanguages: prob.allowed_languages ?? ['python','java','cpp'],
          timeLimitMs: prob.time_limit_ms ?? 2000,
          memoryLimitMb: prob.memory_limit_mb ?? 256,
          maxScore: prob.max_score ?? 100,
        })
      }
    }).catch(() => {}).finally(() => { if (live) setLoadingProblem(false) })
    return () => { live = false }
  }, [assessmentId])
  ...
  const toggleLang = (l) => {
    setForm(f => ({
      ...f,
      allowedLanguages: f.allowedLanguages.includes(l)
        ? f.allowedLanguages.filter(x => x !== l)
        : [...f.allowedLanguages, l],
    }))
  }
  ...
  const handleSave = async (e) => {
    e.preventDefault()
    if (!form.title.trim()) { setErr('Title is required'); return }
    if (!form.problemStatement.trim()) { setErr('Problem statement is required'); return }
    if (!form.sampleInput.trim() || !form.sampleOutput.trim()) { setErr('Sample input/output are required'); return }
    if (form.allowedLanguages.length === 0) { setErr('Select at least one language'); return }
    setBusy(true); setErr(null); setSaved(false)
    try {
      const prob = await upsertCodingProblem({
        assessmentId,
        title: form.title.trim(),
  ...
  if (loadingProblem) return <div className="page-state"><div className="loading-spinner" /></div>

  return (
    <div className="panel">
      <div className="panel-heading">
        <h3>Coding Problem Configuration</h3>
        {existing && <span className="badge badge-APPROVED" style={{ marginLeft: 8 }}>Problem exists</span>}
      </div>

      <div className="alert info" style={{ marginBottom: 16, fontSize: 12.5 }}>
        <strong>Judge0 note:</strong> Execution scoring will be active once Judge0 is configured as a Supabase secret.
        Until then, student submissions are stored for audit and scores must be assigned manually.
      </div>

      {err && <div className="alert error" style={{ marginBottom: 12 }}>{err}</div>}
      {saved && <div className="alert success" style={{ marginBottom: 12 }}>Coding problem saved successfully.</div>}

      <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div className="form-group">
          <label className="form-label">Problem Title *</label>
          <input className="form-input" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Two Sum" />
        </div>

        <div className="form-group">
          <label className="form-label">Problem Statement *</label>
          <textarea className="form-textarea" rows={5} value={form.problemStatement}
            onChange={e => setForm(f => ({ ...f, problemStatement: e.target.value }))}
            placeholder="Describe the problem clearly. Include context, goal, and expected behavior." />
        </div>

        <div className="form-group">
          <label className="form-label">Constraints</label>
          <textarea className="form-textarea" rows={2} value={form.constraintsText}
            onChange={e => setForm(f => ({ ...f, constraintsText: e.target.value }))}
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <textarea className="form-textarea" rows={2} value={form.inputFormat}
              onChange={e => setForm(f => ({ ...f, inputFormat: e.target.value }))}
              placeholder="Describe the input format line by line" />
          </div>
          <div className="form-group">
            <label className="form-label">Output Format</label>
            <textarea className="form-textarea" rows={2} value={form.outputFormat}
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <textarea className="form-textarea" rows={3} value={form.sampleInput}
          ...
          <div className="form-group">
            ...
            <textarea className="form-textarea" rows={3} value={form.sampleOutput}
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 6 }}>
            {['python', 'java', 'cpp', 'javascript'].map(l => (
              <label key={l} className="form-check">
                <input type="checkbox" checked={form.allowedLanguages.includes(l)} onChange={() => toggleLang(l)} />
                <span style={{ fontSize: 13 }}>{l === 'cpp' ? 'C++' : l === 'javascript' ? 'JavaScript' : l.charAt(0).toUpperCase() + l.slice(1)}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-input" type="number" min="500" max="10000" step="500" value={form.timeLimitMs}
              onChange={e => setForm(f => ({ ...f, timeLimitMs: e.target.value }))} />
          </div>
          <div className="form-group">
            <label className="form-label">Memory Limit (MB)</label>
            <input className="form-input" type="number" min="32" max="1024" step="32" value={form.memoryLimitMb}
              onChange={e => setForm(f => ({ ...f, memoryLimitMb: e.target.value }))} />
          </div>
          <div className="form-group">
        ...
        <div>
          <button className="primary-button" type="submit" disabled={busy}>
            {busy ? 'Saving…' : existing ? 'Update Coding Problem' : 'Save Coding Problem'}
          </button>
        </div>
      </form>
    </div>
    ...
    <div>
      ...
      {assessment == null ? (
        ...
        <div>
          ...
          {tab === 'coding' && roundInfo?.round_type === 'CODING' && (
            <CodingProblemEditor assessmentId={assessment.id} />
          )}
        </div>
      )}
    </div>
