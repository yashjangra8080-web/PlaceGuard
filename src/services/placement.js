...
export async function getDashboardData(profile) {
  ...
  const metrics = []
  if (role === 'student') { const { count, error } = await supabase.from('applications').select('*', { count: 'exact', head: true }); if (error) throw error; metrics.push(['Applications', count || 0], ['Notifications', notifications.count || 0]) }
